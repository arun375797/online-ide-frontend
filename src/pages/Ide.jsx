...
function uniqueFileName(files, name, exceptId = null) {
  ...
  const taken = new Set(files.filter((f) => f.id !== exceptId).map((f) => f.name.toLowerCase()));
...
export default function Ide() {
  ...
  useEffect(() => {
    let cancelled = false;
    (async () => {
      const rows = await api("/api/notebooks", { token });
      if (cancelled) return;
      if (!rows.length) {
  ...
  function activeFile(nb = current) {
    if (!nb) return null;
    return nb.files.find((file) => file.id === nb.activeFileId) || null;
  }
  ...
  async function confirmModal() {
    ...
    if (modal.mode === "delete-file") {
      ...
      setModal(null);
      patchCurrent((nb) => {
        const files = nb.files.filter((f) => f.id !== id);
    ...
    if (modal.mode === "file") {
      const name = uniqueFileName(current.files, value);
    ...
    if (modal.mode === "rename") {
      ...
      patchCurrent((nb) => ({
        ...nb,
        files: nb.files.map((f) => (f.id === modal.fileId ? { ...f, name: uniqueFileName(nb.files, value, f.id) } : f)),
      }));
      return;
    }
  ...
  async function importLocalFiles(fileList) {
    ...
    for (const diskFile of fileList) {
      ...
      additions.push({ id: uid(), name: uniqueFileName([...current.files, ...additions], diskFile.name), content });
    }
    if (!additions.length) return;
    patchCurrent((nb) => ({
      ...nb,
      files: [...nb.files, ...additions],
  ...
  return (
    <div className="grid h-full grid-rows-[58px_1fr] grid-cols-[260px_1fr] bg-deep text-white max-[720px]:grid-cols-1">
      <header className="col-span-2 flex items-center gap-4 border-b border-cyan/20 bg-darker px-4">
        <button type="button" className="hidden h-7 w-7 rounded-md border border-cyan/20 max-[720px]:grid place-items-center" onClick={() => setSidebarOpen((v) => !v)}>
          ☰
        </button>
        <Brand />
        <div className="relative">
          <button type="button" className={`rounded-md px-3 py-1.5 text-sm ${menuOpen ? "bg-cyan/15 text-cyan" : "hover:bg-cyan/10"}`} onClick={() => setMenuOpen((v) => !v)}>
            File
          </button>
          {menuOpen ? (
            <div className="absolute left-0 top-full z-20 mt-2 w-56 rounded-xl border border-cyan/20 bg-widget p-1.5 shadow-glow">
              <MenuItem label="New File" kbd="Ctrl+N" onClick={() => { setMenuOpen(false); openModal({ mode: "file", title: "New file", copy: "This file will open in a tab.", ok: "Create", value: uniqueFileName(current.files, "untitled.js") }); }} />
        ...
        <div className="flex items-center gap-2">
          <button type="button" className="hidden rounded-lg border border-cyan/20 px-3 py-2 text-sm hover:border-cyan sm:inline" onClick={() => openModal({ mode: "notebook", title: "New notebook", copy: "A new notebook starts empty.", ok: "Create", value: "Untitled notebook" })}>
            New notebook
          </button>
      ...
      <aside className={`flex min-h-0 flex-col border-r border-cyan/20 bg-deep max-[720px]:absolute max-[720px]:bottom-0 max-[720px]:left-0 max-[720px]:top-[58px] max-[720px]:z-10 max-[720px]:w-[260px] max-[720px]:transition-transform ${sidebarOpen ? "max-[720px]:translate-x-0" : "max-[720px]:-translate-x-[105%]"}`}>
        <section className="p-3">
          <div className="mb-2 flex items-center justify-between">
            <h2 className="text-[11px] font-semibold uppercase tracking-[0.12em] text-muted">Notebooks</h2>
            <button type="button" className="h-6 w-6 rounded border border-cyan/20 hover:border-cyan hover:text-cyan" onClick={() => openModal({ mode: "notebook", title: "New notebook", copy: "A new notebook starts empty.", ok: "Create", value: "Untitled notebook" })}>+</button>
          </div>
          <ul>
            {notebooks.map((nb) => (
              <li key={nb._id}>
        ...
        <section className="flex min-h-0 flex-1 flex-col p-3">
          <div className="mb-2 flex items-center justify-between gap-2">
            ...
            <div className="flex gap-1">
              <button type="button" className="h-6 rounded border border-cyan/20 px-2 text-xs hover:border-cyan hover:text-cyan" onClick={() => fileInputRef.current?.click()}>Open</button>
              <button type="button" className="h-6 w-6 rounded border border-cyan/20 hover:border-cyan hover:text-cyan" onClick={() => openModal({ mode: "file", title: "New file", copy: "This file will open in a tab.", ok: "Create", value: uniqueFileName(current.files, "untitled.js") })}>+</button>
            </div>
          </div>
      ...
      <main className="flex min-h-0 min-w-0 flex-col bg-panel">
        <div className="flex min-h-10 items-stretch border-b border-cyan/20 bg-deep">
          <div className="flex flex-1 overflow-x-auto">
            {openTabs.map((tab) => (
              <button
                key={tab.id}
                type="button"
                className={`flex items-center gap-2 border-r border-cyan/20 px-3 font-mono text-[13px] ${tab.id === current.activeFileId ? "bg-panel text-white shadow-[inset_0_2px_0_#00ffff]" : "text-muted"}`}
          ...
          <button type="button" className="m-1.5 rounded border border-cyan/20 px-2 text-xs hover:border-cyan" onClick={() => fileInputRef.current?.click()}>Open</button>
          <button type="button" className="m-1.5 mr-2 h-[26px] w-[26px] rounded border border-cyan/20 hover:border-cyan" onClick={() => openModal({ mode: "file", title: "New file", copy: "This file will open in a tab.", ok: "Create", value: uniqueFileName(current.files, "untitled.js") })}>+</button>
        </div>
        ...
        <div className="relative min-h-0 flex-1">
          {file && themeReady ? (
            ...
            <div className="grid h-full place-content-center justify-items-center gap-2 bg-panel text-center text-muted">
              <p>{current.files.length ? "No file is open." : "This notebook is empty."}</p>
              <p className="mb-2 text-sm">{current.files.length ? "Open a file from the sidebar, or use File → Open File." : "Create a file or open one from your computer."}</p>
              <div className="flex gap-2">
                ...
                <button type="button" className="rounded-lg bg-lime px-3 py-2 text-sm font-semibold text-deep" onClick={() => openModal({ mode: "file", title: "New file", copy: "This file will open in a tab.", ok: "Create", value: uniqueFileName(current.files, "untitled.js") })}>New file</button>
              </div>
            </div>
          )}
        </div>

        <div className="relative flex flex-col border-t border-cyan/20 bg-overlay" style={{ height: consoleHeight }}>
          <div
            ...
            onMouseDown={(event) => {
              ...
              const move = (ev) => setConsoleHeight(Math.min(Math.max(startH + (startY - ev.clientY), 110), window.innerHeight * 0.55));
              const up = () => {
      ...
      {modal ? (
        <div className="fixed inset-0 z-30 grid place-items-center bg-deep/70" onClick={(e) => e.target === e.currentTarget && setModal(null)}>
          <div className={`w-[min(420px,calc(100%-32px))] rounded-2xl border bg-widget p-5 shadow-glow ${modal.danger ? "border-danger/50" : "border-cyan/20"}`}>
      ...
      {toast ? <div className="fixed bottom-4 right-4 z-40 rounded-xl border border-cyan/20 bg-widget px-4 py-2 text-sm shadow-glow">{toast}</div> : null}
    </div>
  );
}
...
function MenuItem({ label, kbd, onClick }) {
  return (
    <button type="button" className="flex w-full items-center justify-between rounded-lg px-2.5 py-2 text-left text-sm hover:bg-cyan/10 hover:text-cyan" onClick={onClick}>
      <span>{label}</span>
...
function SideItem({ active, label, meta, onClick, onDelete }) {
  return (
    <button type="button" className={`mb-1 flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-left text-sm ${active ? "bg-cyan/15 text-cyan" : "hover:bg-widget"}`} onClick={onClick}>
